...
_path = (function( script, i ){
	...
	for( ; i < l; i++ )
	{
		...
		if( path.substr(path.lastIndexOf('/')).indexOf('dateTime') !== -1 )
		    break;
	}
...
(function(head){
	...
	link.href = _path + 'skins/dateTime.css';
    link.rel = 'stylesheet';
...
var dateTime = function( config )
    {
		...
		return _box ? _box._init(config) : new dateTime.fn._init(config);
	};
...
dateTime.fn._init.prototype = dateTime.fn;

dateTime.formatDate = function( date, format )
{
...
window.dateTime = $.calendar = dateTime;
